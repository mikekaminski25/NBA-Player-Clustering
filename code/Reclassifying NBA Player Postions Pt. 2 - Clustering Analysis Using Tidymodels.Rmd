---
title: "Reclassifying NBA Player Postions Pt. 2 - Clustering Analysis Using Tidymodels"
...
```
- The cummulative vairance is listed at the top of each bar.  The variance captured by each PC is below it <br>
...
## Try kmeans with PCA using less data
...
### Try K-Means with PCA using less data part 3 - only per game statistics
```{r}
...
# Results
K-means w/o PCA: 3 is likely optimal, but 6 or 7 show that it's flattening out <br>
K-means w/ PCA all data: 3 may be optimal, but 7 or 8 show that it's flattening out<br>
K-means w/ PCA 20 minutes per game cutoff (instead of 12): More or less the same as with all data <br>
K-means w/ PCA using only per game stats: 2 may be optimal, levels off around 6 or 7 <br>
Hierarchical w/ PCA: 2 may be optimal, levels off around 6, 7 or 8. 7 or 8 are likely more balanced than 6 <br>
